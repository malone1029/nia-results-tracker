  ...
  async function handleSave(id: number) {
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((i) =>
          i.id === id
            ? { ...i, status: editStatus as FeedbackRow['status'], admin_note: editNote }
            : i
        )
      );
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-text-secondary mt-0.5">
            Bug reports, ideas, and questions from the team
          </p>
        </div>
        ...
        {banner && (
          <div
            className={`mb-4 px-4 py-2 rounded-lg text-sm font-medium ${banner.startsWith('Error') ? 'bg-red-500/10 text-red-600' : 'bg-nia-green/10 text-nia-green'}`}
          >
            {banner}
          </div>
        )}
              ...
              {filtered.map((item) => (
                <div key={item.id}>
                  ...
                  >
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${TYPE_BADGE[item.type]}`}
                    >
                      {item.type}
                    </span>
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${STATUS_BADGE[item.status]}`}
                    >
                      {item.status}
                    </span>
                    <svg
                      className={`w-4 h-4 text-text-muted transition-transform ${expandedId === item.id ? 'rotate-180' : ''}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
          ...
          <div className="sm:hidden space-y-3">
            {filtered.map((item) => (
              <Card key={item.id} className="p-3">
                <button onClick={() => handleExpand(item)} className="w-full text-left">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span
                      className={`ml-auto px-2 py-0.5 rounded-full text-[10px] font-medium ${STATUS_BADGE[item.status]}`}
                    >
                      {item.status}
                    </span>
                  </div>
