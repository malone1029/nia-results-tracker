...
export async function POST() {
  ...
  if (!processes || processes.length === 0) {
    return NextResponse.json({ message: "No processes with legacy Baldrige connections found", converted: 0 });
  }
  ...
  const withContent = processes.filter((p) => {
    ...
    return (questions && questions.length > 0) || (evidence && Object.values(evidence).some(Boolean)) || (content && content.trim().length > 0);
  });

  if (withContent.length === 0) {
    return NextResponse.json({ message: "No processes with meaningful Baldrige connections found", converted: 0 });
  }

  // Fetch all EB questions for matching
  const { data: ebQuestions } = await supabase
    .from("baldrige_questions")
    .select("id, question_code, area_label, question_text, baldrige_items!inner(item_code, item_name)")
    .eq("tier", "excellence_builder")
    .order("sort_order");

  if (!ebQuestions || ebQuestions.length === 0) {
    return NextResponse.json({ error: "No Excellence Builder questions found. Run migration-018 and seed first." }, { status: 400 });
  }
  ...
  const questionRef = ebQuestions.map((q) => {
  ...
  const results: { process_id: number; process_name: string; mappings_created: number; unmatched: string[] }[] = [];
    ...
    try {
      ...
      for (const q of questions) {
        const matched = matches.some((m) => {
          ...
          return ebQ && q.toLowerCase().includes(ebQ.question_code.replace("EB-", "").toLowerCase());
        });
