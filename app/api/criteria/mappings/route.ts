...
export async function POST(request: Request) {
  ...
  if (!process_id || !question_id) {
    return NextResponse.json({ error: 'process_id and question_id are required' }, { status: 400 });
  }
...
export async function PATCH(request: Request) {
  ...
  const { error } = await supabase.from('process_question_mappings').update(updates).eq('id', id);
...
export async function DELETE(request: Request) {
  ...
  const { error } = await supabase.from('process_question_mappings').delete().eq('id', Number(id));
