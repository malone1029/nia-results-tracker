...
function fieldToText(data: Record<string, unknown> | null, orderedKeys?: string[]): string {
  ...
  const keys = orderedKeys
    ? [
        ...orderedKeys,
        ...Object.keys(data).filter((k) => k !== 'content' && !orderedKeys.includes(k)),
      ]
...
async function condenseCharterForAsana(
  ...
  console.log(
    `[Asana Export] Charter is ${charterText.length} chars (limit ${ASANA_NOTES_CHAR_LIMIT}), condensing with AI...`
  );
...
${charterText}`,
    ...
    const condensed = response.content[0].type === 'text' ? response.content[0].text : '';

    if (condensed && condensed.length <= ASANA_NOTES_CHAR_LIMIT) {
      console.log(
        `[Asana Export] Condensed from ${charterText.length} to ${condensed.length} chars`
      );
    ...
    if (condensed && condensed.length > ASANA_NOTES_CHAR_LIMIT) {
      ...
      const truncated =
        condensed.slice(0, ASANA_NOTES_CHAR_LIMIT - 100) +
    ...
    const truncated =
      charterText.slice(0, ASANA_NOTES_CHAR_LIMIT - 100) +
...
async function syncAdliTasks(
  ...
  for (const dimension of dimensions) {
    ...
    console.log(
      `[ADLI Export] ${dimension}: field=${fieldKey}, hasData=${!!rawField}, contentLen=${content.length}, pdcaSection=${pdcaSection}, sectionGid=${sectionGid || 'MISSING'}`
    );
    ...
    if (existingGid) {
      ...
      try {
        ...
        console.warn(
          `[ADLI Export] Existing ${dimension} task (${existingGid}) not found, creating new:`,
  ...
  console.log(
    `[ADLI Export] Result: created=${created}, updated=${updated}, errors=${errors.length}, workspaceGid=${workspaceGid || 'MISSING'}`
  );
...
export async function POST(request: Request) {
  ...
  if (!token) {
    return NextResponse.json(
      { error: 'Asana not connected. Go to Settings to connect.' },
      { status: 401 }
    );
  }
  ...
  try {
    ...
    console.log(
      `[Asana Export] Charter: ${rawCharterText.length} chars raw → ${charterForAsana.length} chars for Asana (condensed: ${wasCondensed})`
    );
    ...
    if (existingGid) {
      try {
        const projCheck = await asanaFetch(
          token,
          `/projects/${existingGid}?opt_fields=name,workspace`
        );
        ...
        if (!workspaceGid) {
          return NextResponse.json(
            {
              error:
                'Could not determine Asana workspace for this project. Try unlinking and re-linking.',
            },
            { status: 400 }
          );
        }
    ...
    if (existingGid) {
      ...
      try {
        ...
        try {
          ...
          console.log(
            `[Asana Export] Notes sent: ${sentLength} chars, stored: ${storedLength} chars`
          );
        ...
        notesWarning =
          'Could not update project notes (you may not have project edit access in Asana). Tasks were synced successfully.';
      }
      ...
      if (wasCondensed) {
        allWarnings.push(
          "Charter was condensed by AI to fit Asana's description limit. The full charter is linked at the top of the project description."
        );
      }
      return NextResponse.json({
        action: 'updated',
      ...
      try {
        const teamsRes = await asanaFetch(token, `/organizations/${workspaceId}/teams?limit=1`);
      ...
      console.log(
        `[Asana Export] syncAdliTasks result: created=${adliResult.created}, updated=${adliResult.updated}, gids=`,
        adliResult.gids
      );
