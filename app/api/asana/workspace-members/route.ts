...
export async function GET() {
  ...
  try {
    ...
    const members = (data.data || []).map((u: { gid: string; name: string; email: string }) => ({
      gid: u.gid,
      name: u.name,
      email: u.email,
    }));
    ...
    // Sort by name
    members.sort((a: { name: string }, b: { name: string }) => a.name.localeCompare(b.name));
    ...
    return NextResponse.json(
      {
        error: 'fetch_failed',
        message: (err as Error).message || 'Failed to fetch workspace members',
      },
      { status: 502 }
    );
  }
