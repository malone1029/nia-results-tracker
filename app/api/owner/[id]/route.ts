...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const processes = [...(byEmail ?? []), ...(byName ?? []).filter((p) => !emailIds.has(p.id))];
  ...
  const metricIds = (metricLinks ?? [])
    .flatMap((ml) => {
      ...
      return Array.isArray(ml.metrics)
        ? (ml.metrics as { id: number }[])
        : [ml.metrics as { id: number }];
    })
    .map((m) => m.id);
  ...
  const [{ data: taskRows }, { data: improvementRows }, { data: baldrigeMappingRows }] =
    await Promise.all([
      processIds.length > 0
        ? supabase
            .from('process_tasks')
            .select('process_id, status, assignee_name, due_date, completed')
            .in('process_id', processIds)
        : {
            data: [] as {
              process_id: number;
  ...
  const { data: adliScores } =
    processIds.length > 0
      ? await supabase
          ...
          .select(
            'process_id, overall_score, approach_score, deployment_score, learning_score, integration_score, assessed_at'
          )
          .in('process_id', processIds)
  ...
  const processHealthScores: number[] = (processes ?? []).map((p) => {
    ...
    return calculateHealthScore(processInput, scoreInput, healthMetricInputs, taskInput, {
  ...
  const improvementCount = (improvementRows ?? []).filter((r) => r.created_at >= yearStart).length;
  ...
  const avgAdliScore =
    processIds.length > 0
      ? Math.round(
          processIds.reduce((sum, id) => sum + (latestAdliByProcess.get(id) ?? 0), 0) /
            processIds.length
        )
      : 0;
  ...
  return NextResponse.json({
    ...
    stewardedMetrics: (stewardedMetrics || []).map((m) => {
      ...
      return {
        ...
        review_status: getReviewStatus(m.cadence, latestEntry?.date ?? null, m.next_entry_expected),
      };
    }),
  });
}
