...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!processId || !action) {
      return Response.json({ error: 'processId and action are required' }, { status: 400 });
    }
    ...
    if (action === 'link') {
      ...
      const { metricId } = body;
      if (!metricId) {
        return Response.json({ error: 'metricId is required for link action' }, { status: 400 });
      }
    ...
    return Response.json({ error: "Invalid action. Use 'link' or 'create'." }, { status: 400 });
  } catch (error) {
    console.error('AI metrics error:', error);
    return Response.json({ error: 'Failed to process metric action' }, { status: 500 });
  }
}
