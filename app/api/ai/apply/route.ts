...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (updateError) {
      ...
      return Response.json({ error: 'Failed to update process' }, { status: 500 });
    }
    ...
    return Response.json({ error: 'Failed to apply suggestion' }, { status: 500 });
  }
}
