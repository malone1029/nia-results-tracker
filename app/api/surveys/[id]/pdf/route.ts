...
export async function POST(
  ...
  const [surveyRes, waveRes, questionsRes, processRes] = await Promise.all([
    ...
    supabase.from("survey_questions").select("id, question_text, question_type, rating_scale_max, options").eq("survey_id", surveyId).order("sort_order"),
    supabase.from("surveys").select("process_id").eq("id", surveyId).single().then(async (s) => {
  ...
  if (responseIds.length > 0) {
    ...
    questionResults = questions.map((q) => {
      ...
      switch (q.question_type) {
        ...
        case "yes_no": {
          const avg = numericValues.length > 0
            ? Math.round((numericValues.filter((v) => v === 1).length / numericValues.length) * 100)
            : 0;
