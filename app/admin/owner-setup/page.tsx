...
export default function OwnerSetupPage() {
  ...
  const [results, setResults] = useState<
    { name: string; email: string; processesUpdated: number; userSeeded: boolean }[] | null
  >(null);
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-text-muted mt-1">
          Map each process owner to their NIA email. This seeds their Hub account and enables
          scorecard matching.
        </p>
      </div>
      ...
      {unmappedCount > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
          {unmappedCount} owner{unmappedCount !== 1 ? 's' : ''} not yet mapped — select an email for
          each row below.
        </div>
      )}
      ...
      {results && (
        <Card className="p-6">
          ...
          <p className="text-xs text-text-muted mt-4">
            Scorecards will now show for all mapped owners. They will see their own scorecard when
            they log in with Google.
          </p>
        </Card>
      )}
    </div>
