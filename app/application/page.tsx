    ...
    <AdminGuard>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div
            className={`bg-card rounded-xl border p-4 ${estPages > 25 ? 'ring-2 ring-nia-red/30' : ''}`}
          >
            <div
        ...
        {categories.map((cat) => {
          ...
          const catDraftedCount = cat.items.filter(
            (item) => draftMap.has(item.id) && draftMap.get(item.id)!.word_count > 0
          ).length;
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-text-muted">
                      {cat.totalPoints} pts &middot; {cat.items.length} items &middot;{' '}
                      {catDraftedCount} drafted &middot; {catDraftWords.toLocaleString()} words
                    </p>
                  </div>
                </div>
                {/* Category progress */}
                <div className="flex items-center gap-2">
                  <div className="w-20 h-1.5 bg-surface-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${cat.items.length > 0 ? (catDraftedCount / cat.items.length) * 100 : 0}%`,
                      }}
                    />
                  </div>
