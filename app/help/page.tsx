      ...
      return (
        <svg className={cn} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            ...
            strokeWidth={1.5}
            d="M15.59 14.37a6 6 0 01-5.84 7.38v-4.8m5.84-2.58a14.98 14.98 0 006.16-12.12A14.98 14.98 0 009.631 8.41m5.96 5.96a14.926 14.926 0 01-5.841 2.58m-.119-8.54a6 6 0 00-7.381 5.84h4.8m2.581-5.84a14.927 14.927 0 00-2.58 5.84m2.699 2.7a18.05 18.05 0 01-.553.832c-.678 1-1.573 1.99-2.635 2.928-.388.342-.815.672-1.274.988a18.05 18.05 0 01-.553-.832c-.678-1-1.573-1.99-2.635-2.928-.388-.342-.815-.672-1.274-.988"
          />
        </svg>
      );
    case 'folder':
      ...
      {search && filteredSections.length === 0 && (
        <Card className="p-8 text-center">
          ...
          <p className="text-sm text-text-muted">
            Try different keywords or submit feedback below.
          </p>
        </Card>
      )}
            ...
            {myFeedback.map((item) => (
              <div key={item.id} className="px-5 py-3">
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${STATUS_BADGE[item.status] || STATUS_BADGE.new}`}
                  >
                    {item.status}
                  </span>
