...
export default function DataHealthPage() {
  ...
  async function fetchMetrics() {
    ...
    rows.sort((a, b) => statusOrder[a.review_status] - statusOrder[b.review_status]);
  ...
  useEffect(() => {
    ...
    async function fetchProcessSummary() {
      const { data } = await supabase.from('processes').select('process_type, status');
      if (data) {
  ...
  async function bulkUpdate() {
    ...
    setMetrics((prev) =>
      prev.map((m) => (ids.includes(m.id) ? { ...m, [bulkField]: updateValue } : m))
    );
    setSuccessMessage(
      `Updated ${fieldLabel} for ${ids.length} metric${ids.length !== 1 ? 's' : ''}`
    );
  ...
  const displayMetrics = displayMetricsAll.filter(
    (m) => stewardFilter === 'all' || m.data_steward_email === stewardFilter
  );
  ...
  return (
    <div className="space-y-6 content-appear">
      ...
      <SectionIntro storageKey="intro-data-health">
        Track which metrics are current and which need attention. Green = healthy, orange = due
        soon, red = overdue. Use &apos;Edit Metrics&apos; for bulk changes.
      </SectionIntro>
        ...
        <Card accent="orange" padding="sm">
          <div className="space-y-3">
            <div className="flex items-center gap-2 overflow-x-auto pb-1">
              ...
              {(['overdue', 'due-soon', 'no-data', 'current', 'scheduled'] as const).map(
                (status) => {
                  const count = displayMetrics.filter((m) => m.review_status === status).length;
                  if (count === 0) return null;
                  const statusIds = displayMetrics
                    .filter((m) => m.review_status === status)
                    .map((m) => m.id);
        ...
        <Card variant="elevated" padding="md" className="flex items-center gap-5">
          ...
          <div>
            ...
            <div className="text-sm text-text-muted mt-0.5">
              {displayMetrics.length - noData.length} of {displayMetrics.length} metrics tracked
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-nia-dark mt-1">
                ...
                <span className="text-text-muted text-sm">
                  {' '}
                  of {processSummary.total} processes marked as key
                </span>
              </p>
            </div>
            <div className="flex gap-4 text-center">
              {processSummary.keyApproved > 0 && (
                <div>
                  <div className="text-lg font-bold text-nia-green">
                    {processSummary.keyApproved}
                  </div>
              ...
              {processSummary.keyInProgress > 0 && (
                <div>
                  <div className="text-lg font-bold text-nia-orange">
                    {processSummary.keyInProgress}
                  </div>
        ...
        <Card accent="orange" padding="md">
          <div ref={logFormRef}>
            <h3 className="font-bold text-nia-dark mb-4">
              Log Value: {metrics.find((m) => m.id === logForm.metricId)?.name}
            </h3>
            <form onSubmit={handleLogSubmit} className="space-y-4">
              ...
              <Input
                ...
                onChange={(e) => setLogForm({ ...logForm, noteAnalysis: e.target.value })}
              ...
              <div className="flex gap-3">
                ...
                <Button variant="secondary" type="button" onClick={() => setLogForm(null)}>
                  Cancel
                </Button>
              </div>
            </form>
  ...
  return (
    <div className="relative w-28 h-28 flex-shrink-0">
      <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
        <circle cx="50" cy="50" r={radius} fill="none" stroke="var(--grid-line)" strokeWidth="7" />
        <circle
          cx="50"
          cy="50"
          r={radius}
...
function HeroCard({
  ...
  return (
    <Card
      variant="elevated"
      accent={accentMap[color] || 'none'}
      padding="md"
      className={`flex flex-col justify-center ${value > 0 && glowMap[color] ? glowMap[color] : ''}`}
    >
...
function MetricSection({
  ...
  return (
    <Card
      accent={accentColor ? accentMap[accentColor] || 'none' : 'none'}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-text-tertiary">
                      {metric.category_display_names} &middot; {metric.process_names} &middot;{' '}
                      {metric.cadence}
                    </div>
                      ...
                      metric.review_status === 'overdue'
                        ...
                        : metric.review_status === 'due-soon'
                          ...
                          : metric.review_status === 'current'
                            ? 'green'
                            : metric.review_status === 'scheduled'
                              ? 'blue'
                              : 'gray'
                    }
                    size="xs"
