...
function ProcessOwnerDashboard() {
  ...
  const [overdueMetrics, setOverdueMetrics] = useState<
    { id: number; name: string; processOwner: string | null }[]
  >([]);
  const [dueSoonMetrics, setDueSoonMetrics] = useState<
    { id: number; name: string; processOwner: string | null }[]
  >([]);
  ...
  const [recentImprovements, setRecentImprovements] = useState<
    { processId: number; processName: string; label: string; date: string; status: string }[]
  >([]);
    ...
    async function fetchAll() {
      ...
      const [
        ...
        supabase
          ...
          .select('process_id, title, created_at, status')
      ...
      const computedOrgAvgHealth =
        orgTotalWeight > 0 ? Math.round(orgWeightedSum / orgTotalWeight) : 0;
      ...
      if (fullName) {
        ...
        if (match) {
          ...
          fetch(`/api/tasks/dashboard?owner=${encodeURIComponent(match)}`)
            ...
            .then((data) => {
              if (data) setDashboardTasks(data);
            })
            .catch(() => {});
        } else if (!isAdmin) {
      ...
      const metricProcessLinks = (metricProcessLinksRes.data || []) as {
        metric_id: number;
        process_id: number;
      }[];
      ...
      const improvements = (improvementsRes.data || []) as {
        process_id: number;
        title: string;
        created_at: string;
        status: string;
      ...
      if (uid && (forceWelcome || !hasCompletedOnboarding(uid))) {
        const urlAsana =
          new URLSearchParams(window.location.search).get('asana_connected') === 'true';
  ...
  const healthLevel =
    healthCount > 0
      ? avgHealth >= 80
        ? { label: 'Excellence Ready', color: '#b1bd37' }
        : avgHealth >= 60
          ? { label: 'On Track', color: '#55787c' }
          : avgHealth >= 40
            ? { label: 'Developing', color: '#f79935' }
  ...
  const needsAttentionCount = filteredProcesses.filter((p) => {
    ...
    const stale = lastDate
      ? (now - new Date(lastDate).getTime()) / (1000 * 60 * 60 * 24) > 60
      : false;
  ...
  const normalize = (s: string) =>
    s
      .replace(/\bfor this process\b/gi, '')
      .replace(/\bthis process\b/gi, '')
      .replace(/\s+\(last updated \d+ days ago\)/i, '')
      .trim()
      .toLowerCase();
  const actionGroups = new Map<
    string,
    { label: string; points: number; count: number; href?: string }
  >();
  ...
  const topNextActions = [...actionGroups.values()].sort((a, b) => b.points - a.points).slice(0, 3);
    ...
    <div className="space-y-6 content-appear">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-text-tertiary mt-1">
            {!isAll && userName ? `${userName}'s Excellence Hub` : 'Organization-wide overview'}
          </p>
        </div>
        {isAdmin && (
      ...
      {processCount === 0 ? (
        !isAll ? (
          ...
          <Card padding="lg">
            <div className="text-center mb-6">
              <h2 className="text-xl font-bold text-[#324a4d] mb-1">
                Get Started with Your First Process
              </h2>
              <p className="text-sm text-text-tertiary">
                Choose how you&apos;d like to add your first process to the Excellence Hub.
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl mx-auto">
              ...
              >
                ...
                <div>
                  <div className="text-sm font-semibold text-[#324a4d] group-hover:text-[#55787c]">
                    Import from Asana
                  </div>
              ...
              >
                ...
                <div>
                  <div className="text-sm font-semibold text-[#324a4d] group-hover:text-[#f79935]">
                    Create with AI
                  </div>
                  <div className="text-xs text-text-muted mt-0.5">
                    AI builds it from your answers
                  </div>
                </div>
              </Link>
              <Link
              ...
              >
                ...
                <div>
                  <div className="text-sm font-semibold text-[#324a4d] group-hover:text-text-secondary">
                    Create Manually
                  </div>
            ...
            <div className="text-center mt-5">
              <Link
                href="/processes"
                className="text-xs text-[#55787c] hover:text-[#324a4d] underline underline-offset-2"
              >
        ...
        <>
          ...
          {isAll && <RecentActivity improvements={recentImprovements} />}
        </>
      )}
    </div>
  );
