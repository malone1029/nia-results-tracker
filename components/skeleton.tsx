...
function TableSkeleton({ rows = 5 }: { rows?: number }) {
  return (
    <div className="bg-card rounded-lg shadow overflow-hidden">
      {/* Header */}
      <div className="bg-surface-hover px-4 py-3 flex gap-6">
      ...
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="px-4 py-3 border-t border-border-light flex gap-6">
...
export function FormSkeleton({ fields = 5 }: { fields?: number }) {
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <PageHeaderSkeleton />
      <div className="bg-card rounded-lg shadow p-6 space-y-5">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="bg-card rounded-lg shadow p-3">
