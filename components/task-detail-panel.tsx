...
export default function TaskDetailPanel({
  ...
  useEffect(() => {
    ...
    fetch(`/api/tasks/${task.id}/activity`)
      .then((r) => (r.ok ? r.json() : []))
      .then((data) => {
        if (isMountedRef.current) setActivities(data);
      })
      .catch(() => {});
  ...
  const filteredMembers = members
    .filter((m) => m.name && m.name.toLowerCase().includes(mentionSearch))
    .slice(0, 5);
    ...
    return parts.map((part, i) => {
      if (part.match(/^@[A-Za-z]+ [A-Za-z]+$/)) {
        return (
          <span
            key={i}
            className="text-nia-grey-blue font-medium bg-nia-grey-blue/10 rounded px-0.5"
          >
            {part}
          </span>
        );
  ...
  function formatActivity(a: TaskActivity): string {
    ...
    switch (a.action) {
      ...
      case 'completed':
        return `${name} marked complete`;
      case 'uncompleted':
        return `${name} marked incomplete`;
      case 'deleted':
              ...
              <div className="space-y-1">
                ...
                {blockedBy.map((dep) => (
                  <div
                    ...
                    className="flex items-center gap-2 bg-surface-hover rounded-lg px-2.5 py-1.5"
                  >
                    <span
                      className={`w-2 h-2 rounded-full flex-shrink-0 ${dep.completed ? 'bg-nia-green' : 'bg-red-500'}`}
                    />
                    <span
                      className={`text-xs flex-1 min-w-0 truncate ${dep.completed ? 'line-through text-text-muted' : 'text-nia-dark'}`}
                    >
                      {dep.title}
                    </span>
                    <button
            ...
            {blocking.length > 0 && (
              <div className="space-y-1">
                ...
                {blocking.map((dep) => (
                  ...
                  >
                    <span
                      className={`w-2 h-2 rounded-full flex-shrink-0 ${dep.completed ? 'bg-nia-green' : 'bg-amber-500'}`}
                    />
                    <span
                      className={`text-xs flex-1 min-w-0 truncate ${dep.completed ? 'line-through text-text-muted' : 'text-nia-dark'}`}
                    >
          ...
          <div className="space-y-2">
            ...
            {showRecurrenceEditor ? (
                ...
                {recType === 'monthly' && (
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={recDayOfMonth}
                      onChange={(e) =>
                        setRecDayOfMonth(Math.max(1, Math.min(31, Number(e.target.value))))
                      }
                      className="w-14 text-xs text-center bg-card border border-border-light rounded-lg px-2 py-1 text-foreground focus:outline-none focus:border-nia-grey-blue"
                    />
                  </div>
              ...
              <div className="flex items-center justify-between bg-surface-hover rounded-lg px-3 py-2">
                <div className="flex items-center gap-2">
                  <svg
                    className="w-3.5 h-3.5 text-nia-grey-blue"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <span className="text-xs text-nia-dark">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (task.recurrence_rule!.dayOfWeek !== undefined)
                      setRecDayOfWeek(task.recurrence_rule!.dayOfWeek);
                    if (task.recurrence_rule!.dayOfMonth !== undefined)
                      setRecDayOfMonth(task.recurrence_rule!.dayOfMonth);
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      {att.url ? (
                        ...
                        <span className="text-xs text-nia-dark truncate block">
                          {att.file_name}
                        </span>
                      )}
                ...
                <div className="absolute bottom-full left-0 right-0 mb-1 bg-card border border-border rounded-lg shadow-lg max-h-40 overflow-y-auto z-10">
                  {filteredMembers.map((m, idx) => (
                    ...
                    >
                      ...
                      {m.email && <span className="ml-2 text-text-muted">{m.email}</span>}
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="space-y-2">
            ...
            <div className="text-xs text-text-muted space-y-1">
              ...
              {task.last_synced_at && (
                <p>
                  Last synced:{' '}
                  {new Date(task.last_synced_at).toLocaleDateString('en-US', {
                    month: 'short',
                    day: 'numeric',
                    year: 'numeric',
                  })}
