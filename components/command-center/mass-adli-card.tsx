          ...
          try {
            ...
            if (event.type === "progress") {
              ...
              setRecentScored((prev) => [
                { processId: event.processId, processName: event.processName, overall: event.overall },
    ...
    <Card padding="md">
      ...
      <div className="flex items-start gap-3 mb-4">
        <div className="w-9 h-9 rounded-lg bg-nia-dark/10 flex items-center justify-center flex-shrink-0">
          <svg className="w-5 h-5 text-nia-dark" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Scope selector */}
      <div className="mb-4">
        <p className="text-xs font-medium text-text-secondary uppercase tracking-wider mb-2">Scope</p>
          ...
          {recentScored.length > 0 && (
            <div className="mt-3 space-y-1">
              {recentScored.slice(0, 5).map((r) => (
                <div key={r.processId} className="flex items-center justify-between py-1 px-2 rounded bg-card text-xs">
                  <span className="text-nia-dark truncate mr-2">{r.processName}</span>
                  <span className={`font-medium whitespace-nowrap ${
                    r.overall >= 70 ? "text-nia-dark" :
                    r.overall >= 50 ? "text-nia-green" :
                    r.overall >= 30 ? "text-nia-orange" : "text-nia-red"
                  }`}>
                    {r.overall}%
                  </span>
                </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-surface-hover border-b border-border">
                    <th className="text-left py-2 px-3 text-xs font-medium text-text-secondary">Category</th>
                    <th className="text-right py-2 px-3 text-xs font-medium text-text-secondary">Avg Score</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={row.category} className="border-b border-border-light last:border-0">
                        ...
                        <td className="py-2 px-3 text-right">
                          <span className={`font-medium ${
                            row.avg >= 70 ? "text-nia-dark" :
                            row.avg >= 50 ? "text-nia-green" :
                            row.avg >= 30 ? "text-nia-orange" : "text-nia-red"
                          }`}>
                            {row.avg}%
                          </span>
                        </td>
