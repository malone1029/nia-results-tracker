...
export default function ClassifyProcessesCard() {
  ...
  async function runClassification() {
    ...
    try {
      ...
      const diffIds = new Set(
        suggestions.filter((r) => r.suggestion !== r.current_type).map((r) => r.process_id)
      );
      setSelected(diffIds);
      setStatus('ready');
    } catch {
  ...
  function selectChanged() {
    setSelected(
      new Set(results.filter((r) => r.suggestion !== r.current_type).map((r) => r.process_id))
    );
  }
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-surface-hover border-b border-border">
                    ...
                    <th className="text-left py-2 px-3 text-xs font-medium text-text-secondary hidden md:table-cell">
                      Category
                    </th>
                    <th className="text-left py-2 px-3 text-xs font-medium text-text-secondary">
                      Current
                    ...
                    <th className="text-left py-2 px-3 text-xs font-medium text-text-secondary hidden lg:table-cell">
                      Rationale
                    </th>
                  </tr>
                </thead>
                  ...
                  {results.map((r) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3 font-medium text-nia-dark">{r.name}</td>
