...
export function InputNode({ data }: { data: IOData }) {
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Top} style={{ opacity: 0, transform: "skewX(8deg)" }} />
      <span style={{ transform: "skewX(8deg)", lineHeight: 1.3 }}>{data.label}</span>
      <Handle type="source" position={Position.Bottom} style={{ opacity: 0, transform: "skewX(8deg)" }} />
    </div>
  );
}
