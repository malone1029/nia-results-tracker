...
import SidebarHealthWidget, { type SidebarHealthData } from '@/components/sidebar-health-widget';
...
const superAdminNavGroups = [
  {
    ...
    links: [
      ...
      { href: '/admin/owner-setup', label: 'Owner Setup', icon: 'link' },
    ],
  },
];
...
export default function Sidebar({
  ...
  function renderGroup(
    ...
    return (
      <div key={group.label}>
        {/* Group header — hidden when collapsed at md */}
        <button
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-sidebar">
      ...
      >
        ...
        {isAdmin && adminNavGroups.map((group) => renderGroup(group, true))}
        {role === 'super_admin' && superAdminNavGroups.map((group) => renderGroup(group, true))}
      </nav>
  ...
  return (
    <>
      {/* Desktop sidebar — collapsed icons at md, full at lg */}
      <aside className="hidden md:flex md:flex-col md:fixed md:inset-y-0 md:left-0 md:w-16 lg:w-60 md:z-40">
        {sidebarContent}
      </aside>
      ...
      {open && (
        <>
          ...
          {/* Drawer */}
          <aside className="md:hidden fixed inset-y-0 left-0 w-60 z-50 sidebar-enter">
            {sidebarContent}
          </aside>
        </>
      )}
