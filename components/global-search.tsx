...
function highlightMatch(text: string, query: string) {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <span className="font-semibold text-nia-orange">{text.slice(idx, idx + query.length)}</span>
...
export default function GlobalSearch({
    ...
    const timeout = setTimeout(async () => {
      ...
      const [procRes, metricRes, reqRes] = await Promise.all([
        ...
        supabase
          ...
          .or(`name.ilike.${pattern},description.ilike.${pattern},data_source.ilike.${pattern}`)
          .limit(8),
        supabase
  ...
  const flatResults = [...visibleProcesses, ...visibleMetrics, ...visibleRequirements];
  ...
  const noResults =
    ...
    filtered.metrics.length === 0 &&
    filtered.requirements.length === 0;
  ...
  const allFilterTabs: { key: FilterKey; label: string; count: number }[] = [
    {
      key: 'all',
      label: 'All',
      count: results.processes.length + results.metrics.length + results.requirements.length,
    },
          ...
          return (
            <button
              ...
              className={`w-full text-left px-3 py-2.5 flex flex-col transition-colors ${
                idx === highlightIndex ? 'bg-nia-grey-blue/20' : 'hover:bg-surface-hover'
              }`}
            ...
            >
              ...
              <span className="text-xs text-text-muted mt-0.5">{item.subtitle}</span>
            </button>
          );
        })}
        {hiddenCount > 0 && (
  ...
  return (
    <div ref={containerRef} className={`relative ${mobile ? 'w-full' : 'w-64'}`}>
      ...
      {/* Results dropdown */}
      {open && (
        <div className="absolute top-full mt-1 left-0 w-full max-w-[580px] bg-card rounded-lg shadow-lg border border-border z-[60] overflow-hidden flex flex-col">
          {/* Filter tabs */}
          ...
          <div className="max-h-[420px] overflow-y-auto">
            {loading && (
              <div className="px-3 py-4 text-sm text-text-muted text-center">Searching…</div>
            )}
