...
export default function ImprovementStepper({
  currentStep,
  onStepClick,
  onAction,
}: ImprovementStepperProps) {
    ...
    <div className="bg-card rounded-xl shadow-sm border border-border-light overflow-hidden">
      ...
      <div className="px-4 sm:px-5 pt-4 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg bg-nia-dark/5 flex items-center justify-center">
            <svg
              className="w-4 h-4 text-nia-dark"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
          ...
          <span className="text-xs font-semibold text-nia-dark uppercase tracking-wider">
            Improvement Cycle
            <HelpTip text="Your 6-step guided cycle. Complete each step to strengthen your process documentation." />
          </span>
        </div>
        {isComplete ? (
      ...
      {onAction && !isComplete && (
        <div className="px-4 sm:px-5 pb-4 pt-1 border-t border-border-light">
          <StepActionButtons step={STEPS[currentIndex]} onAction={onAction} />
        </div>
      )}
    </div>
  );
