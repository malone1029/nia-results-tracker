...
async function upsertMetric(
  ...
  const { data: existing } = await supabase.from('metrics').select('id').eq('name', name).single();
...
async function seedEntry(metricId: number, value: number, date: string, noteAnalysis?: string) {
  const { data: existing } = await supabase
    .from('entries')
    .select('id')
...
async function seed() {
  ...
  const leadershipScorecard = await upsertMetric(
    strategicDirection,
    'Leadership Scorecard Components',
    {
      cadence: 'quarterly',
  ...
  await seedEntry(ldiWellOrganized, 5.0, '2023-11-09', 'First Team, Onboarding');
  await seedEntry(ldiWellOrganized, 5.0, '2024-02-01', '5 Whys, Interview questions');
  await seedEntry(ldiWellOrganized, 5.0, '2024-04-16', 'Financial literacy, Feedforward');
  await seedEntry(ldiWellOrganized, 5.0, '2024-08-13', 'Difficult conversations');
  await seedEntry(ldiWellOrganized, 5.0, '2024-09-19', 'Asana, Performance convos');
  ...
  await seedEntry(ldiImportant, 5.0, '2023-11-09');
  await seedEntry(ldiImportant, 5.0, '2024-02-01');
  await seedEntry(ldiImportant, 5.0, '2024-04-16');
  await seedEntry(ldiImportant, 5.0, '2024-08-13');
  await seedEntry(ldiImportant, 4.94, '2024-09-19');
  await seedEntry(ldiImportant, 4.85, '2024-11-07');
  await seedEntry(ldiImportant, 5.0, '2025-01-30');
  ...
  const roundingCompletion = await upsertMetric(
    alwaysActions,
    'Rounding Completion Rate (SLT/ELT)',
    {
      cadence: 'monthly',
  ...
  await seedEntry(bssTechMean, 4.8, '2024-04-01', 'Spring 2024');
  ...
  const attorneyConsult = await upsertMetric(
    policyMgmt,
    'Policies Requiring Attorney Consultation',
    {
      cadence: 'annual',
  ...
  await seedEntry(
    ...
    -1095000,
    '2025-06-30',
    'FY25 deficit — largest in recent history'
  );
  ...
  const avgRateVariance = await upsertMetric(
    rateDev,
    'Average Rate Variance (Required vs Approved)',
    {
      cadence: 'annual',
      data_source: 'Rate analysis',
  ...
  await seedEntry(
    ...
    -38.19,
    '2025-06-30',
    'Average shortfall per day across services'
  );
  ...
  await seedEntry(
    ...
    -877747,
    '2025-06-30',
    'FY24-FY25 combined gap from ad hoc rate decisions'
  );
  ...
  const leParentNotification = await upsertMetric(
    lawEnforcement,
  ...
  await seedEntry(mdsMean, 4.9, '2025-03-01', 'Mar 2025');
