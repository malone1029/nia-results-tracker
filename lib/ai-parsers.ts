...
export function parseCoachSuggestions(text: string): {
...
export function parseMetricSuggestions(text: string): {
...
export function parseSurveyQuestions(text: string): {
  questions: SurveyQuestionSuggestion[];
  cleanedText: string;
} {
...
export function hasPartialBlock(
  text: string
): 'scores' | 'suggestions' | 'tasks' | 'metrics' | 'survey-questions' | null {
